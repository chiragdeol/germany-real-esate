...
if ($method === 'GET') {
  ...
  if ($db) {
    try {
      ...
      // If settings exist, map them to content structure
      if (!empty($settingsMap)) {
        $heroStats = isset($settingsMap['hero_stats']) ? json_decode($settingsMap['hero_stats'], true) : $defaultContent['hero']['stats'];
