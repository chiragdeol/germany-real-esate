  ...
  const renderMarkdown = (text: string) => {
    ...
    return text.split("\n\n").map((block, idx) => {
      ...
      if (trimmed.startsWith("###")) {
        return (
          <h4 key={idx} className="font-display text-xl font-bold text-foreground mt-8 mb-3">
            {trimmed.replace(/^###\s+/, "")}
          </h4>
        );
      }
      if (trimmed.startsWith("##")) {
        return (
          <h3 key={idx} className="font-display text-2xl font-bold text-foreground mt-10 mb-4">
            {trimmed.replace(/^##\s+/, "")}
          </h3>
        );
      }
      if (trimmed.startsWith("#")) {
        return (
          <h2 key={idx} className="font-display text-3xl font-bold text-foreground mt-12 mb-6">
            {trimmed.replace(/^#\s+/, "")}
          </h2>
        );
      }
